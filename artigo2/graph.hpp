...
#include <queue>
#include <limits>
#include "imageloader.hpp" // Inclua o arquivo com a função loadPPM
...
public:
    ...
    void addEdge(int u, int v, double cap) {
        capacity[u][v] += cap;
        adj[u].push_back(v);
        adj[v].push_back(u); // Para busca reversa
    }

    // Função para executar o algoritmo de max-flow usando Edmonds-Karp
    double maxFlow() {
    ...
    while (!q.empty()) {
        ...
        for (int v : adj[u]) {
            ...
            if (!visited[v] && capacity[u][v] > 1e-6) { // Pequeno valor para evitar problemas numéricos
                visited[v] = true;
                q.push(v);
            }
        }
